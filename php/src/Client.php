...
namespace APIVerve\Imagecaption;
 ...
 * ```
 *
 * @see https://apiverve.com/marketplace/imagecaption?utm_source=php&utm_medium=readme
 */
class Client
{
    private const BASE_URL = 'https://api.apiverve.com/v1/imagecaption';
     ...
     */
    public function executeWithFile(string $filePath, array $params = []): array
    {
        ...
        try {
            ...
            throw new APIException("File upload failed: " . $e->getMessage(), $e->getCode());
        }
    }
     ...
     */
    public function executeWithUrl(string $url, array $params = []): array
    {
        if (!filter_var($url, FILTER_VALIDATE_URL)) {
            throw new \InvalidArgumentException("Invalid URL format");
        }

        return $this->execute(array_merge(['url' => $url], $params));
    }
    ...
    {
        if ($this->debug) {
            echo "[APIVerve\ImagecaptionClient] {$message}\n";
        }
    }
}
