...
class ImagecaptionAPIClientError(Exception):
...
class ValidationError(ImagecaptionAPIClientError):
...
class ImagecaptionAPIClient:
    # Validation rules for parameters (generated from schema)
    VALIDATION_RULES = {"image": {"type": "string", "required": True}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the ImagecaptionAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises ImagecaptionAPIClientError: If API key is invalid
        """
        ...
        self.secure = secure
        self.debug = debug
        self.base_url = 'https://api.apiverve.com/v1/imagecaption'
        self.headers = {
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise ImagecaptionAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[ImagecaptionAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns ImageCaptionData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises ImagecaptionAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise ImagecaptionAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise ImagecaptionAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise ImagecaptionAPIClientError(f"Request failed: {str(e)}")

    def execute_with_file(self, files: Dict[str, Any], params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        Execute the API request with a file upload

        :param files: Dictionary of files to upload, e.g., {"image": open("image.jpg", "rb")}
        :param params: Additional form parameters (optional)
        :return: API response as a dictionary
        :raises ImagecaptionAPIClientError: If the request fails or returns an error
        """

        self._log(f"Making POST request with file upload to {self.base_url}")
        if files:
            self._log(f"Files: {list(files.keys())}")
        if params:
            self._log(f"Parameters: {params}")
        ...
        try:
            ...
            self._log("Sending POST request with multipart/form-data")
            ...
            self._log("File upload successful")
            return response_data
            ...
            self._log(f"Request exception: {str(e)}")
            raise ImagecaptionAPIClientError(f"File upload failed: {str(e)}")

    def execute_with_file_path(self, file_path: str, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        Execute the API request with a file upload from a file path
        ...
        with open(file_path, 'rb') as f:
            ...
            return self.execute_with_file(files, params)

    def execute_with_url(self, url: str, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        ...
        if not url or not isinstance(url, str):
            raise ImagecaptionAPIClientError("URL must be provided as a string.")

        if not re.match(r'^https?://.+', url, re.IGNORECASE):
            raise ImagecaptionAPIClientError("Invalid URL format. URL must start with http:// or https://")
