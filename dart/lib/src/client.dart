...
class ImagecaptionValidationException implements Exception {
  final List<String> errors;

  ImagecaptionValidationException(this.errors);

  @override
  String toString() => 'ImagecaptionValidationException: ${errors.join("; ")}';
}
...
/// Image Caption API client.
///
/// For more information, visit: https://apiverve.com/marketplace/imagecaption?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [image] (required) - Upload an image file to generate a caption (supported formats: JPG, PNG, GIF, max 5MB)
class ImagecaptionClient {
  ...
  ImagecaptionClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/imagecaption',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [ImagecaptionValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw ImagecaptionValidationException(errors);
    }
  }

  /// Execute a request to the Image Caption API.
  ///
  /// Parameters are validated before sending the request.
  Future<ImagecaptionResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw ImagecaptionException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return ImagecaptionResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw ImagecaptionException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw ImagecaptionException('Resource not found');
      } else {
        throw ImagecaptionException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is ImagecaptionException) rethrow;
      throw ImagecaptionException('Request failed: $e');
    }
  }

  /// Execute a request to the Image Caption API with a file upload.
  ///
  /// [filePath] - Path to the file to upload
  /// [additionalParams] - Optional additional parameters to send with the request
  Future<ImagecaptionResponse> executeWithFile(
    String filePath, {
    ...
    try {
      ...
      if (!await file.exists()) {
        throw ImagecaptionException('File not found: $filePath');
      }
      ...
      if (e is ImagecaptionException) rethrow;
      throw ImagecaptionException('File upload failed: $e');
    }
  }
  ...
  Future<ImagecaptionResponse> executeWithBytes(
    List<int> bytes,
    String fileName, {
    Map<String, String>? additionalParams,
  }) async {
...
/// Exception thrown by the Image Caption API client.
class ImagecaptionException implements Exception {
  final String message;

  ImagecaptionException(this.message);

  @override
  String toString() => 'ImagecaptionException: $message';
}
